...
nnoremap <leader>d<BS> :call vimspector#DeleteWatch()<Cr>
...
function! MochaStrategy(cmd)
  let testName = matchlist(a:cmd, '\(.*\)mocha \(test.*\.spec\.js\)')[2]

  try
    let testGrep = matchlist(a:cmd, '\(.*\)mocha \(test.*\.spec\.js\) --grep \(.*\)')[3]
